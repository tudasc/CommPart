...
Microtask::Microtask(CallInst *fork_call)
{
    ...
    for (auto &argument : _function->args())
    {
        ...
        if (argument.getArgNo() > 1)
        {
            ...
            Value* in_serial=_fork_call->getArgOperand(start_args_at_fork+argument.getArgNo());
    ...
    {
        if (call->getCalledFunction()->getName().equals("__kmpc_for_static_init_4"))
        {
    ...
    for (auto &call : call_instructions)
    {
        ...
        else if (call->getCalledFunction()->getName().equals("__kmpc_end_reduce_nowait"))
        {
...
std::vector<std::pair<llvm::Value *,llvm::Value *>> &Microtask::get_shared_variables() { return _shared_variables; }
